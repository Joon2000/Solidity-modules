...
3. 내부적 동작
- `mapping`은 키와 값의 쌍을 블록체인 상에 저장하지 않고, 키의 해시 값을 이용해 저장합니다. 따라서 전체 키-값 쌍을 열거하는 것은 불가능하며, 특정 키를 통해서만 값에 접근할 수 있습니다.
...
이 컨트랙트는 중첩 매핑을 사용하여 주소를 키로 하고, 그 주소에 대해 또 다른 매핑(정수형 `uint256`을 키로 하고 불리언 값을 저장하는 매핑)을 저장합니다.
...
- 입력된 주소 `_addr1`와 정수 `_i`에 대한 중첩 매핑 값을 디폴트 값으로 재설정합니다. 이는 `delete`를 사용하여 이루어집니다.
